...
(* When s is negative, OR with signed_min doesn't change the MSB (it's already 1) *)
...
Proof.
  ...
  { apply bv_or_size; [reflexivity | rewrite Hs; apply signed_min_size]. }
  ...
  - assert (Hn0 : n = 0%N).
    { 
      ...
      rewrite N2Nat.id in En.
      exact En.
    }
    
    assert (s = []).
    { 
      pose proof (bits_size s) as Hlen.
      unfold bits in Hlen.
      rewrite Hs in Hlen.
      rewrite Hn0 in Hlen.
      simpl in Hlen.
  ...
  - unfold signed_min.
    ...
    assert (Hrev_false : rev (mk_list_false n0) = mk_list_false n0).
    { apply rev_mk_list_false. }
    
    rewrite Hrev_false.
    
    assert (Hlen_s : length s = S n0).
    { 
      pose proof (bits_size s) as H.
      unfold bits in H.
      rewrite Hs in H.
      rewrite En in H.
      exact H.
    ...
    assert (Hs_decomp : exists s_init, s = s_init ++ [true] /\ length s_init = n0).
    {
      ...
      - exists st.
        split.
        + unfold bits in Hlast.
          rewrite last_last in Hlast.
          rewrite Hlast.
          reflexivity.
        + rewrite app_length in Hlen_s.
          simpl in Hlen_s.
          lia.
    }
    ...
    assert (Hlen_eq : length s_init = length (mk_list_false n0)).
...
Proof.
  ...
  - unfold signed_min.
    ...
    assert (Hs_decomp : exists s_init, s = s_init ++ [false] /\ length s_init = n0).
    {
      destruct s as [| sh st] using rev_ind.
      - simpl in Hlen_s.
        discriminate Hlen_s.
    ...
    + assert (Hac_size : size a = size c).
      {
        ...
        rewrite Hab_size, Hbc_size.
        reflexivity.
      }
      
      apply N.eqb_eq in Hac_size.
      rewrite Hac_size.
      unfold sle_list in *.
...
Proof.
  ...
  - simpl in Hlast_a.
    discriminate Hlast_a.
    
  - destruct (bits b) eqn:Hbits_b.
    + assert (length (bits a) = N.to_nat (size a)) by apply bits_size.
      assert (length (bits b) = N.to_nat (size b)) by apply bits_size.
      rewrite Hbits_a in H.
      rewrite Hbits_b in H0.
      simpl in H, H0.
      
      assert (N.to_nat (size a) = N.to_nat (size b)).
    ...
    + assert (Hhd_a : hd false (rev (bits a)) = true).
      { rewrite hd_rev. rewrite Hbits_a. exact Hlast_a. }
      
      assert (Hhd_b : hd false (rev (bits b)) = false).
      { rewrite hd_rev. rewrite Hbits_b. exact Hlast_b. }
      
      rewrite Hbits_a in Hhd_a.
      rewrite Hbits_b in Hhd_b.
      
      destruct (rev (b0 :: l)) eqn:Hrev_a.
      * assert (b0 :: l = []).
        { apply (f_equal (@rev bool)) in Hrev_a.
          rewrite rev_involutive in Hrev_a.
          exact Hrev_a. }
        discriminate H.
      ...
      * destruct (rev (b1 :: l0)) eqn:Hrev_b.
        -- assert (b1 :: l0 = []).
           { apply (f_equal (@rev bool)) in Hrev_b.
             rewrite rev_involutive in Hrev_b.
             exact Hrev_b. }
           discriminate H.
           
        -- simpl in Hhd_a, Hhd_b.
           subst b2 b3.
...
(* If either operand has MSB=1, result has MSB=1 *)
...
Proof.
  ...
  assert (Hlen: length (bits a) = length (bits b)).
  { rewrite (bits_size a), (bits_size b). rewrite Ha, Hb. reflexivity. }
  unfold bv_or.
  assert (Hsize_eq : size a =? size b = true).
  { apply N.eqb_eq. rewrite Ha, Hb. reflexivity. }
  rewrite Hsize_eq.
  remember (bits a) as la.
  ...
  - rewrite last_last in Hlast_a.
    ...
    assert (Hlen_init : length la = length lb).
    {
      do 2 rewrite app_length in Hlen.
      simpl in Hlen.
      apply Nat.add_cancel_r in Hlen.
      exact Hlen.
    }
    rewrite Hlast_a.
    rewrite map2_or_app; try exact Hlen_init; try reflexivity.
    unfold bits. 
    simpl.
    rewrite last_last.
    reflexivity.
...
Proof.
  intros a b.
  unfold bv_sge, bv_sle.
  
  destruct (size a =? size b) eqn:Hsize.
  - apply N.eqb_eq in Hsize.
    assert (Hsize_flipped : (size b =? size a) = true).
    { apply N.eqb_eq. symmetry. exact Hsize. }
    rewrite Hsize_flipped.
    unfold sge_list, sle_list.
    
    assert (Hlen : length (rev a) = length (rev b)).
    { 
      do 2 rewrite length_rev.
      pose proof (bits_size a) as Hlen_a.
      pose proof (bits_size b) as Hlen_b.
      rewrite Hsize in Hlen_a.
      rewrite <- Hlen_b in Hlen_a.
      exact Hlen_a.
    }
    ...
    + simpl.
      ...
      match goal with
      | |- context[eqb ?x ?y] => destruct x; destruct y
      end.
      
      * simpl.
        destruct (uge_list_big_endian _ _) eqn:Huge;
  ...
  - apply N.eqb_neq in Hsize.
    assert (Hsize_flipped : (size b =? size a) = false).
    { apply N.eqb_neq. intro Hcontra. apply Hsize. symmetry. exact Hcontra. }
    rewrite Hsize_flipped.
    reflexivity.
Qed.
...
Proof.
  ...
  - destruct b; [ | discriminate Hlen ].
    simpl. reflexivity.

  - destruct b as [|y ys]; [ discriminate Hlen | ].
    simpl in Hlen. injection Hlen as Hlen_tails.
    simpl. 
...
(* s | signed_min is a lower bound for x | s in signed comparison *)
...
Proof.
  ...
  { 
    ...
    rewrite Hsize_left, Hsize_right.
    reflexivity. 
  }
  ...
  - rewrite last_last in Hleft_msb.
    ...
    assert (H_goal : sle_list_big_endian (rev (la ++ [x0])) (rev (lb ++ [x1])) = true).
    {
      ...
      assert (Hrev_map2_or : forall a b, length a = length b -> 
                       rev (map2 orb a b) = map2 orb (rev a) (rev b)).
      {
        intros a b Hlen_ab.
        apply nth_ext with (d := false) (d' := false).
        - rewrite rev_length.
          assert (H1 : length (map2 orb a b) = length a).
      ...
      assert (H_full_or : true :: rev lb = map2 orb (true :: rev la) (rev (bits x))).
      {
        ...
        {
          unfold bv_or, bits.
          rewrite Hsize_x, Hsize_s, N.eqb_refl.
          reflexivity.
        }
        ...
        {
          unfold bv_or, bits.
          rewrite Hsize_s, signed_min_size, N.eqb_refl.
          reflexivity.
        }
        ...
        {
          unfold bits.
          pose proof (bits_size x) as H1.
          pose proof (bits_size s) as H2.
          unfold bits in H1, H2.
          rewrite Hsize_x in H1.
          rewrite Hsize_s in H2.
        ...
        {
          unfold bits.
          pose proof (bits_size s) as H1.
          pose proof (bits_size (signed_min n)) as H2.
          unfold bits in H1, H2.
          rewrite Hsize_s in H1.
          rewrite signed_min_size in H2.
          rewrite H1, H2. reflexivity.
        }
        
        rewrite Hrev_map2_or by exact Hlen_xs.
        rewrite Hrev_map2_or by exact Hlen_s_min.
        rewrite <- map2_or_assoc.
        rewrite (map2_or_comm (rev (bits (signed_min n))) (rev (bits x))).
        rewrite map2_or_assoc.
        rewrite (map2_or_comm (rev (bits x)) (rev (bits s))).
        symmetry.
        ...
        {
          apply bv_or_signed_min_neg.
          - assert (Hsize_xs : size (bv_or x s) = n).
            { apply bv_or_size; [exact Hsize_x | exact Hsize_s]. }
            exact Hsize_xs.
          - exact H_xs_neg.
        }

        apply (f_equal bits) in H_or_idem.
        unfold bits in H_or_idem.
        unfold bv_or in H_or_idem at 1.
        assert (Hsize_xs : size (bv_or x s) = n).
        { apply bv_or_size; [exact Hsize_x | exact Hsize_s]. }
        rewrite Hsize_xs, signed_min_size, N.eqb_refl in H_or_idem.
        rewrite Hbv_or_xs in H_or_idem.
        ...
        assert (Hlen_map2_xs : length (map2 orb (bits x) (bits s)) = length (bits (signed_min n))).
        {
          ...
          { symmetry. apply map2_or_length. exact Hlen_xs. }
          rewrite H1, Hlen_xs.
          exact Hlen_s_min.
        }
        ...
        rewrite Hbv_or_xs in H_or_idem.
        rewrite Hrev_map2_or in H_or_idem by exact Hlen_xs.
        rewrite (map2_or_comm (rev (bits x)) (rev (bits s))) in H_or_idem.
        exact H_or_idem.
      }

      destruct (rev (bits x)) as [| x_msb x_lower] eqn:Hrev_x.
      { 
        apply (f_equal (@length bool)) in Hrev_x. 
        simpl in Hrev_x. 
        rewrite rev_length, bits_size, Hsize_x in Hrev_x. 
        lia. 
      }
      simpl in H_full_or.
      injection H_full_or as H_lb_structure.
      rewrite H_lb_structure.
      ...
      { 
        rewrite Heqla, bits_size. 
        f_equal. 
        apply bv_or_size; [exact Hsize_s | apply signed_min_size]. 
      }
      
      rewrite app_length in Hlen_la. simpl in Hlen_la.
      simpl in Hlen_x. rewrite rev_length. lia.
    }
    
    rewrite Heqla in H_goal.
    rewrite Heqlb in H_goal.
    exact H_goal.
Qed.
