...
**Invertibility conditions proof time:** 40 hours, 7 minutes, 14 seconds (~2 hours, 13 minutes, 44 seconds on average)  
**Dependent type lift time:** ~36 minutes and 34 seconds (38 theorems lifted from `InvCond.v` to `DepInvCond.v`)
...
| `HANDOFF.md`     | A handoff document written mid-project to preserve the state of `bvmult_sgt` after Claude Code hit a session token limit. When the limit was reached, a compaction request was issued but the summary was denied, causing all session context to be lost. This file was written proactively to capture the proof skeleton, two in-progress helper lemmas, and the one-line fix needed to unblock compilation, so work could resume in a new session. |
...
| 8       | bvmult_sge (cont.), bvudiv_uge, bvudiv_reverse_neq, bvudiv_reverse_sgt, bvudiv_reverse_sge, bvurem_slt, bvurem_sle, bvurem_reverse_eq, bvurem_reverse_sgt, bvurem_reverse_sge; **DepInvCond.v: 38 theorems lifted to dependent bitvector types** | `08-bvmult-sge-bvudiv-bvurem-dep-invcond.jsonl` | Yes         |
